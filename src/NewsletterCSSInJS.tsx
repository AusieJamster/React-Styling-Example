...
const Bar = styled.div`
  ...
  height: ${(props: { active: boolean }) => (props.active ? '100%' : '0.5em')};
  width: 20%;
...
function NewsletterCSSInJS(props: {}) {
    ...
    <Container>
      <Spectrum aria-hidden>
        {Array.from(Array(5)).map((_, i) => (
          <Bar active={i + 1 <= emailPartsCount} key={i} />
        ))}
      </Spectrum>
      <Header>
      ...
      />
      <Submit active={emailPartsCount >= 5} disabled={emailPartsCount < 5}>
        Sign up
      </Submit>
    </Container>
  )
...
export default NewsletterCSSInJS
